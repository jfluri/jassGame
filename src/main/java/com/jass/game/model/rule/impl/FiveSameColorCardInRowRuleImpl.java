...
public class FiveSameColorCardInRowRuleImpl implements JassRuleMather {
    ...
    public boolean isMatch(final Player player, final CardColor trumpColor) {
        return player.getPlayCards()
                ...
                .values()
                .stream()
                .anyMatch(this::isFiveConsecutive);
    }

    private boolean isFiveConsecutive(final List<Card> cards) {
        final Set<CardType> cardTypes = cards.stream()
                .map(Card::getCardType)
                .collect(Collectors.toSet());

        final CardType[] allCardTypes = CardType.values();
        for (int i = 0; i < allCardTypes.length - 4; i++) {
            if (hasConsecutiveFromIndex(allCardTypes, i, cardTypes, 5)) {
                return true;
            }
        }
